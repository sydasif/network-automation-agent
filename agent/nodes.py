...
execute_node = ToolNode([show_command, config_command, plan_task, respond])


def understand_node(state: dict[str, Any]) -> dict[str, Any]:
    """Process user input and route to appropriate tools.

    Handles context window management via message trimming and summarization.
    When history exceeds token limit, older messages are summarized to retain
    critical context (device names, issues, actions) while freeing up space.
