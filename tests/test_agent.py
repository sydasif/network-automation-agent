...
def test_understand_node_basic(mock_llm):
    """Test that understand_node processes a basic message."""
    # Mock the LLM to return a predictable response
    # We need to mock the get_llm_with_tools function in agent.nodes
    with pytest.MonkeyPatch.context() as m:
        m.setattr("agent.nodes.get_llm_with_tools", lambda: mock_llm)
...
def test_understand_node_trimming():
    """Test that message trimming logic works (mocking the trimmer if needed)."""
...
def test_planner_node(mock_llm):
    ...
    with pytest.MonkeyPatch.context() as m:
        m.setattr("agent.nodes.get_llm", lambda: mock_llm)
